...
const InputNavigation = ({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>): void => {
    ...
    if (selectedIndex > 0 && inputElement?.selectionStart === 0 && e.key === "ArrowLeft") {
      // Navigate left
      e.preventDefault();
      setDirection("left");
      setSelectedIndex(selectedIndex - 1);
    } else if (selectedIndex !== -1 && selectedIndex < value.length && inputElement?.selectionStart === inputValue.length && e.key === "ArrowRight") {
      // Navigate right
      e.preventDefault();
      setDirection("right");
      setSelectedIndex(selectedIndex === value.length - 1 ? -1 : selectedIndex + 1);
    } else if (inputElement?.selectionStart === 0 && e.key === "ArrowLeft") {
      // Switch to input navigation
      e.preventDefault();
      setDirection("left");
      setSelectedIndex(value.length - 1);
    } else if (selectedIndex !== 0 && inputElement?.selectionStart === 0 && e.key === "Backspace") {
      // Delete tag
      if(selectedIndex === - 1) {
  ...
  useEffect(() => {
    if (inputRef.current) {
      inputRef.current.selectionStart = direction === "left" ? 0 : inputValue.length;
      inputRef.current.selectionEnd = direction === "left" ? 0 : inputValue.length;
  ...
  return (
    ...
    >
      ...
      <span ref={mirrorRef} className="taglist-hidden-span">
        {inputValue || ' '} {/* Fallback to a single space to ensure minimum width */}
      </span>
    </div>
  );
};
